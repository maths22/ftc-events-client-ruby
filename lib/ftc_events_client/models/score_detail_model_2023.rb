...
module FtcEventsClient
  class ScoreDetailModel2023
    attr_accessor :match_level

    attr_accessor :match_series
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FtcEventsClient::ScoreDetailModel2023` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FtcEventsClient::ScoreDetailModel2023`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'match_series')
        self.match_series = attributes[:'match_series']
      end
      ...
      if attributes.key?(:'randomization')
        self.randomization = attributes[:'randomization']
      end
    ...
    def hash
      [match_level, match_series, match_number, randomization, alliances].hash
    end
