...
module FtcEventsClient
  class ScoreDetailSinglePlayer2022
    attr_accessor :init_signal_sleeve
    ...
    attr_accessor :dc_terminal

    attr_accessor :eg_navigated
    ...
    # Array of 4 cone counts scored by this team on ground, low, medium, and high junctions respectively, scored in autonomous. E.g. red.autoJunctionCones[2] is the total number of cones scored by red on medium-height junctions.
    attr_accessor :auto_junction_cones

    # Array of 4 cone counts scored by this team on ground, low, medium, and high junctions respectively, scored in driver-controlled. E.g. red.dcJunctionCones[2] is the total number of cones scored by red on medium-height junctions.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FtcEventsClient::ScoreDetailSinglePlayer2022` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FtcEventsClient::ScoreDetailSinglePlayer2022`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'dc_terminal')
        self.dc_terminal = attributes[:'dc_terminal']
      end
    ...
    def hash
      [init_signal_sleeve, robot_auto, auto_terminal, auto_junctions, dc_junctions, dc_terminal, eg_navigated, minor_penalties, major_penalties, auto_navigation_points, signal_bonus_points, auto_junction_cone_points, auto_terminal_cone_points, dc_junction_cone_points, dc_terminal_cone_points, ownership_points, circuit_points, eg_navigation_points, auto_points, dc_points, endgame_points, penalty_points_committed, pre_penalty_total, auto_junction_cones, dc_junction_cones, beacons, owned_junctions, circuit, total_points].hash
    end
