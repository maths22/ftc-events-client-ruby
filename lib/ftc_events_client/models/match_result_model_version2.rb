...
module FtcEventsClient
  class MatchResultModelVersion2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FtcEventsClient::MatchResultModelVersion2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FtcEventsClient::MatchResultModelVersion2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [actual_start_time, description, tournament_level, series, match_number, score_red_final, score_red_foul, score_red_auto, score_blue_final, score_blue_foul, score_blue_auto, post_result_time, teams, modified_on].hash
    end
