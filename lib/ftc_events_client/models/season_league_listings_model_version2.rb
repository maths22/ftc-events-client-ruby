...
module FtcEventsClient
  class SeasonLeagueListingsModelVersion2
    attr_accessor :leagues

    attr_accessor :league_count
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FtcEventsClient::SeasonLeagueListingsModelVersion2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FtcEventsClient::SeasonLeagueListingsModelVersion2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'leagues')
        if (value = attributes[:'leagues']).is_a?(Array)
          self.leagues = value
        end
      end

      if attributes.key?(:'league_count')
        self.league_count = attributes[:'league_count']
      end
    end
